    ...
    name: 'App',
      ...
      changeBase64ToLatex: function(html, doc, fn){
        ...
        $.each(imgs, function(i, item){
            var $item = $html.find("img.kfformula").eq(i);
            ...
            $('<span class="kfformula-latex" style="position:relative;display:'+display+';padding:0 10px;' + _style + '">'+latex+'</span>').insertAfter($item);
        });
        $html.find("img.kfformula").remove();
        var outerhtml = $html.html();
        if(fn && typeof fn == "function"){
          fn(outerhtml);
          this.$nextTick(()=>{
            window.MathJax.Hub.Queue(["Typeset", MathJax.Hub, doc]);
          });
        }
      }
    },
